...
- “目标原子”输入框保存的是待应用的草稿；仅输入或修改文本不会改变已有结果。
- 点击“应用范围并分析”后，所选原子范围才会一次性应用到数据表、统计、二维图、范围导出和三维电荷着色。输入为空表示分析全部原子。
- 片段统计属于可选的高级分析；不定义片段也能正常计算，片段还可以引用当前分析范围之外的原子。
- “范围结果”导出当前已应用范围，“完整原始结果”导出体系的全部原子数据。
- 三维视图始终保留完整结构和成键关系，仅目标原子使用 Bader 电荷转移颜色及其色标范围。
- 三维场景按工作区和版本安全缓存。切换表格、二维图和三维视图不会重复构建结构；应用新的原子范围通常只更新颜色。更换体系、结构源文件发生变化或相关版本修订时缓存失效并重建。

完整变更见 [v0.2.0 发布说明](docs/releases/v0.2.0.md)。
...
dist/BaderChargeAnalyzer_Setup_v0.2.0.exe
```
...
导入文件说明：导入 Bader 计算之后得到的 `CONTCAR`、`POTCAR` 以及 `ACF.dat` 文件进行计算解析。
